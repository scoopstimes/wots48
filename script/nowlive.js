...
card.appendChild(liveType);
  ...
  if (Array.isArray(member.streaming_url_list)) {
    member.streaming_url_list.forEach(urlObj => {
      if (urlObj.label === 'original quality' && urlObj.url) {
        ...
        const link = `showroom2.html#url=${encodeURIComponent(urlObj.url)}&name=${encodeURIComponent(member.name)}&viewers=${viewers}&start_date=${startDate}&type=${encodeURIComponent(member.type)}&link=${encodeURIComponent(url)}`;
  ...
  if (Array.isArray(member.streaming_url_list)) {
    member.streaming_url_list.forEach((urlObj) => {
     ...
     const link = `idn2.html#url=${ProxyUrl}${encodeURIComponent(urlObj.url)}&name=${encodeURIComponent(member.name)}&viewers=${viewers}&start_date=${startDate}&type=${encodeURIComponent(member.type)}&slug=${encodeURIComponent(cleanedSlug)}&link=${encodeURIComponent(url)}&image=${member.img}`;
     fullscreenBtn.setAttribute('onclick', `goToLink('${link}')`);
        cardBody.appendChild(fullscreenBtn);
      }
    });
