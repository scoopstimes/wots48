  ...
  if (Array.isArray(member.streaming_url_list)) {
    member.streaming_url_list.forEach(urlObj => {
      if (urlObj.label === 'original quality' && urlObj.url) {
        ...
        const link = `showroom.html#url=${encodeURIComponent(urlObj.url)}&name=${encodeURIComponent(member.name)}&viewers=${viewers}&start_date=${startDate}&type=${encodeURIComponent(member.type)}&link=${encodeURIComponent(url)}`;
